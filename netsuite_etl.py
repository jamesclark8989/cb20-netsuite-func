    ...
    for row in items:
        ...
        cursor.execute(
            ...
            INSERT INTO ItemDim (
                ...
                ItemType, IsInactive, UnitCost, SalePrice, LastModifiedDate
            ) VALUES (?,?,?,?,?,?,?,?,?)
            """,
            row.get("id"),
...
def load_sales_orders(conn):
    ...
    for row in details:
        ...
        cursor.execute(
            ...
            safe_decimal(row.get("grossamt")),
            safe_decimal(row.get("estgrossprofit")),
            safe_decimal(row.get("estgrossprofitpercent")),
            safe_decimal(row.get("costestimate")),
            safe_decimal(row.get("taxrate1")),
            safe_decimal(row.get("tax1amt")),
    ...
    for row in headers:
        ...
        cursor.execute(
            ...
            INSERT INTO SalesOrderFulfillment_Header (
                NetSuiteID, TranID, TransactionNumber, CreatedFromID, CreatedFromType,
    ...
    for row in headers:
        ...
        cursor.execute(
            ...
            INSERT INTO SalesOrderInvoice_Header (
                NetSuiteID, TranID, TransactionNumber, CreatedFromID, CreatedFromType,
                SalesOrderID, CustomerID, TranDate, DueDate, Status,
...
def load_purchase_order_receipts(conn):
    ...
    for row in headers:
        ...
        cursor.execute(
            ...
            INSERT INTO PurchaseOrderReceipt_Header (
                NetSuiteID, TranID, TransactionNumber, CreatedFromID, CreatedFromType,
                PurchaseOrderID, VendorID, TranDate, Status, Memo,
                CreatedDate, LastModifiedDate
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)
            """,
            row.get("id"),
    ...
    skipped = 0
    for row in details:
        receipt_id = row.get("itemreceipt")
        if not receipt_id:
        ...
        cursor.execute(
            ...
            INSERT INTO PurchaseOrderReceipt_Detail (
                ...
                Quantity, QuantityRemaining, Rate, Amount, Location
            ) VALUES (?,?,?,?,?,?,?,?,?)
            """,
            receipt_id,
            safe_int(row.get("line")),
...
def load_purchase_order_vendor_bills(conn):
    ...
    for row in headers:
        ...
        cursor.execute(
            ...
            INSERT INTO PurchaseOrderVendorBill_Header (
                NetSuiteID, TranID, TransactionNumber, CreatedFromID, CreatedFromType,
                PurchaseOrderID, VendorID, TranDate, DueDate, Status,
                Total, TaxTotal, AmountDue, AmountPaid,
                Terms, Memo, Currency, Voided,
                CreatedDate, LastModifiedDate
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    ...
    skipped = 0
    for row in details:
        vendor_bill_id = row.get("vendorbill")
        ...
        cursor.execute(
            ...
            INSERT INTO PurchaseOrderVendorBill_Detail (
                VendorBillID, Line, ItemID, Description,
                Quantity, Amount, Rate, TaxRate1, Tax1Amt,
                Class, Department
            ) VALUES (?,?,?,?,?,?,?,?,?,?,?)
            """,
